...
public class EditClothesType implements Command {
    private static final Logger userLogger = LogManager.getLogger(EditClothesType.class);

    @Override
    public void execute(HttpServletRequest request, HttpServletResponse response, File uploadFilePath) throws ServletException, IOException {

        Integer id = Integer.valueOf(request.getParameter("typeId"));
        String catalogType = request.getParameter("editCategory");

        ServiceProvider provider = ServiceProvider.getInstance();
        ClothesTypeService clothesTypeService = provider.getClothesTypeService();
        ClothesType clothesType = new ClothesType(id, catalogType);

        try {
            clothesTypeService.edit(clothesType);
            response.sendRedirect("Controller?command=go_to_types_page&message=message.editType.complete");
